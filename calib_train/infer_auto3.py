...
K1S,K2S=0.167,0.240; TW,TH=512,288; L,S,M=34.0,26,5.0
...
def run(path):
    ...
    q=order4(quad)
    # KAMERA-ÇAPALI: yakın köşe = undist'te en altta (max y); roller ona göre
    ni=int(np.argmax(q[:,1])); C0=q[ni]; nb1=q[(ni+1)%4]; nb2=q[(ni-1)%4]; diag=q[(ni+2)%4]
    if np.linalg.norm(nb1-C0)>=np.linalg.norm(nb2-C0): cfN,cnF=nb1,nb2   # uzun kenar=yakın taç
    else: cfN,cnF=nb2,nb1
    cfF=diag
    edges=[(C0,cnF),(cfN,cfF),(C0,cfN),(cnF,cfF)]  # goalN,goalF,touchN,touchF (kamera=C0=goalN∩touchN)
    ...
    if best is not None:
        ...
        circ=cp
    circ=None   # auto-yuvarlak güvenilmez (yuvarlağı olmayan sahada yanlış aspect) -> aspect=varsayım 0.53
    ...
    lb=[0.0,0.0]+[-np.inf]*8+[17.9,0.5]; ub=[0.45,0.7]+[np.inf]*8+[18.1,6.0]
    ...
    def MXl(X,Y): return [ (M+X)*S, WS-(M+Y)*S ]      # kamera SOL-alt
    def MXr(X,Y): return [ (M+L-X)*S, WS-(M+Y)*S ]    # kamera SAĞ-alt (ayna)
    # ALPEREN KURALI: uzak kale (goalF=src[2],src[3]) görüntüde SOLDA ise kamera SAĞ, sağda ise SOL
    fgx=(src[2][0]+src[3][0])/2.0
    mirrored=(fgx<cx); MX=MXr if mirrored else MXl; camside="SAG-alt" if mirrored else "SOL-alt"
    ...
    cam=MX(0,0); cv2.circle(top,tuple(map(int,cam)),10,(0,200,255),-1); cv2.putText(top,"KAMERA",(int(cam[0])+8,int(cam[1])-6),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,200,255),2)
    ...
    cv2.putText(top,f"2D kam={camside} res{fit:.2f}m",(8,28),cv2.FONT_HERSHEY_SIMPLEX,0.65,(0,230,255),2)
    return montage(path,vis,top)

def montage(path,vis,top):
    # correct_2d.jpg gibi: SADECE temiz 2D paneli
    nm=os.path.basename(path).split('__')[0]
    cv2.putText(top,nm[:22],(8,top.shape[0]-12),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),2)
    Hh=720; top=cv2.resize(top,(int(top.shape[1]*Hh/top.shape[0]),Hh))
    return top
