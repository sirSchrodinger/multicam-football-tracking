...
OUT=os.path.join(os.path.dirname(os.path.abspath(__file__)),os.environ.get("OUTDIR","seg2_dark_ckpt")); os.makedirs(OUT,exist_ok=True)
...
def to_t(img,m):
    ...
    return x,y
def aug(img):  # ÇOK güçlü gece aug (dark-variant): agresif gamma + ekstrem-karartma + high-ISO gürültü
    f=img.astype(np.float32)
    g=np.random.uniform(0.28,1.7); f=(((f/255.)**g)*255.)          # daha düşük gamma tabanı
    f=f*np.random.uniform(0.6,1.25)+np.random.uniform(-16,16)
    if np.random.rand()<0.30:                                       # %30 EKSTREM karanlık (b~5-30 sahaları taklit)
        f=f*np.random.uniform(0.12,0.40)
        f=f+np.random.normal(0,np.random.uniform(6,16),f.shape)     # yüksek-ISO okuma gürültüsü
        f=np.clip(f,0,255)
        f=np.random.poisson(np.clip(f,0,None)*0.6)/0.6              # foton-shot gürültüsü (sinyal-bağımlı)
    else:
        f=f+np.random.normal(0,5,f.shape)
