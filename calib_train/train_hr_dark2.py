...
def to_t(img,m):
    ...
    return x,y
def aug(img):  # DARK2: kanıtlanmış ağırlıklı-BCE reçetesi + GÜÇLÜ gece-aug (clahe YOK, inference hr2 gibi kalır)
    f=img.astype(np.float32)
    g=np.random.uniform(0.30,1.6); f=(((f/255.)**g)*255.)
    f=f*np.random.uniform(0.6,1.22)+np.random.uniform(-14,14)
    if np.random.rand()<0.32:                                   # %32 EKSTREM karanlık (b~5-30 taklit)
        f=f*np.random.uniform(0.12,0.40)
        f=np.clip(f,0,255)+np.random.normal(0,np.random.uniform(6,15),f.shape)   # yüksek-ISO okuma gürültüsü
        f=np.random.poisson(np.clip(f,0,None)*0.6)/0.6                            # foton-shot gürültüsü
    else:
